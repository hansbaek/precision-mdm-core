...
- 작업자 : Hans
- **상세 모달 딥링크(`?view=<id>`)** (사용자 영향 있음 — 개선)
  - 시험항목 **상세(읽기 전용) 모달**의 열림 상태를 URL 쿼리에 동기화. 특정 항목 상세를 **북마크·공유·새로고침**해도 다시 열리고, **브라우저 뒤로가기로 모달이 닫힘**.
  - URL을 단일 소스로 두고 보고 있는 항목을 목록에서 파생 — 닫기는 `replace`라 뒤로가기 시 모달이 되살아나지 않음. 목록에 없는 id로 진입하면 조용히 무시(graceful).
  - 편집/생성/삭제 모달은 휘발성이라 URL 동기화 대상이 아님. 경로 정규화 시 쿼리스트링 보존(`App.tsx`).
  - 테스트 3종 추가(URL 반영·딥링크 진입·미존재 id) — `use-std-items-dashboard`. (web 40→42)
- **세션 강제 종료를 soft-redirect로 전환** (사용자 영향 있음 — 개선)
  - 토큰 만료(401·리프레시 실패) 시 `document.location.href`로 **전체 페이지를 새로고침**하던 것을 SPA 내 전환으로 변경 — 번들 재다운로드·깜빡임 제거, 체감 속도 개선.
  - 강제 로그아웃 시 **스토어(인증·프로필·권한)·환경설정 초기화 + 쿼리 캐시 비움**으로 이전 사용자 응답이 다음 세션에 남지 않도록 함(공유 PC 정보 잔존 방지). 동시 다발 401에도 안내 토스트·전환은 1회만.
  - 저수준 api 계층은 전역 이벤트만 발행하고, 라우터 안 워처가 정리+전환을 담당(`use-force-logout`). 테스트 2종 추가. (web 38→40)
- **StdTestItem 편집 모달 분할** (내부 개선, 동작 변화 없음)
  - 1,225줄짜리 거대 컴포넌트를 **193줄 셸**로 축소. 상태·옵션·저장 로직은 `useStdTestItemForm` 훅으로, 화면은 섹션 컴포넌트(기본정보·시험정의·시장·등록정보 등)로 `std-test-item-form/`에 분리.
  - 폼 로직 단위 테스트 5종 추가(초기값·필드/시장 토글·저장·승인 대기·오류). (web 33→38)
...
- **하이브리드 API 테스트 전략 도입** — DB 없이 도는 e2e(인증·권한·검증, mock)와, 핵심 Oracle 쿼리를 실제 DB로 검증하는 opt-in `*.db-spec`(Testcontainers) 분리. `pnpm test`는 DB 불필요. 상세: 개발 [api/README.md](../api/README.md)
